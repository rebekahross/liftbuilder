...
const fetchActiveWorkout = async (userId) => {
  ...
  const formattedWorkout = await formatWorkoutForFrontend(data.id, userId);
...
const formatWorkoutForFrontend = async (workoutId, userId) => {
  ...
  const exercisesWithSets = await Promise.all(workoutExercises.map(async (workoutExercise) => {
...
const fetchWorkoutById = async (workoutId, userJwt) => {
  ...
  // Format for frontend
  const formattedWorkout = await formatWorkoutForFrontend(workoutId, userId);
...
const insertWorkout = async (workoutData, userJwt) => {
  ...
  if (workoutData.exercises && Array.isArray(workoutData.exercises)) {
    await Promise.all(workoutData.exercises.map(async (exercise, index) => {
...
const modifyWorkout = async (workoutId, updates, userId) => {
  ...
  if (updates.exercises && Array.isArray(updates.exercises)) {
    ...
    await Promise.all(updates.exercises.map(async (exerciseUpdate, index) => {
      ...
      if (existingExercise) {
        ...
        if (exerciseUpdate.sets && Array.isArray(exerciseUpdate.sets)) {
          ...
          await Promise.all(exerciseUpdate.sets.map(async (setUpdate, setIndex) => {
    ...
    if (existingExercises) {
      const exerciseIdsToKeep = updates.exercises
        .filter(e => e.id)
        .map(e => e.id);
      ...
      if (exercisesToRemove.length > 0) {
        await supabase
          .from('workout_exercises')
          .delete()
          .in('id', exercisesToRemove.map(e => e.id));
      }
    }
  }
...
const markWorkoutComplete = async (workoutId, completionData, userId) => {
  ...
  if (completionData.exercises && Array.isArray(completionData.exercises)) {
    ...
    await Promise.all(completionData.exercises.map(async (exerciseData) => {
